...
— title: заголовок события по-русски, до 90 знаков
— gist: суть в 1–2 предложениях по-русски, только факты из материалов
— category: одно из CULTURE, ARCHITECTURE, CINEMA, MUSIC, FASHION, SPORT, GENERAL
...
— skip: true, если материал каналу не подходит. Тогда добавь skipReason.
  ...
  · локальные новости США без международного веса: кантри-музыка, местное ТВ,
    награды второго ряда. Аудитория канала — Москва, ей это не интересно;
  · карточки проектов из архитектурных каталогов — описания частных домов,
    квартир, интерьеров кафе и офисов вида «casa tertulia от такого-то бюро»
    или «дом на склоне в коста-рике». Таких публикуют десятками в день, это
    портфолио, а не новость. Архитектуру берём, только когда есть событие:
    открылось заметное здание, музей или публичное пространство, известное
    бюро выиграло конкурс, здание сносят или вокруг него скандал.
...
export async function clusterNewItems(maxItems: number = MAX_PER_RUN): Promise<ClusterResult> {
  ...
  for (const [batchIndex, batch] of batches.entries()) {
    ...
    for (const cluster of response.clusters ?? []) {
      ...
      // на первом прогоне «Человек-паук» так завёлся дважды.
      if (!existing) {
        ...
        const twin = recent.find((s) => similarity(words, keywords(s.title)) >= SAME_STORY_THRESHOLD);
        if (twin) {
