...
const SUMMARY_SYSTEM =
  'Ты — новостной редактор. Пересказываешь новости по-русски коротко, точно и ' +
  'без украшательств. Никогда не добавляешь фактов, которых нет в исходных ' +
  'материалах.';
...
export async function writeDraft(storyId: string): Promise<DraftPayload | null> {
  ...
  const prompt = [
    ...
    '',
    'Правила:',
    '— 2–4 предложения, до 500 знаков.',
    '— Только факты из материалов. Ничего не додумывай.',
    '— Назови конкретику: кто, что, когда, где, цифры и даты, если они есть.',
    '— Без оценок, без рекламных оборотов, без markdown и эмодзи.',
    '— Не пиши заголовок, только сам пересказ.',
    '— Имена и названия оставляй латиницей как в оригинале.',
