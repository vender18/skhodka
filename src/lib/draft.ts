...
export async function writeDraft(storyId: string): Promise<DraftPayload | null> {
  ...
  const prompt = [
    ...
    '— Не превращай действие в цитату. Если в источнике сказано, что человек\n'
      + '      что-то СДЕЛАЛ, нельзя писать «по его словам» — это разные вещи.\n'
      + '      Цитируй только то, что в источнике стоит в кавычках.',
    '— Живость добавляй ИНТОНАЦИЕЙ, а не новыми фактами. Нельзя дописывать\n'
      + '      слухи, ожидания и прогнозы, которых нет в источнике: «шуршат слухи\n'
      + '      о дропе», «ждём осенью», «фанаты в предвкушении» — так текст\n'
      + '      перестаёт быть правдой. Если даты или цены нет — так и пиши.',
