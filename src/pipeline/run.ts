 ...
 * Два режима работы:
 *
 *   collect — раз в час. Забирает свежее, разбирает на инфоповоды и сразу
 *             отправляет только то, что горит. Остальное копится.
 *   digest  — несколько раз в день. Берёт накопленное, выстраивает план
 *             публикаций и присылает пачку готовых черновиков.
 */
...
/** Сколько черновиков максимум в одном дайджесте. */
const DIGEST_SIZE = 8;
